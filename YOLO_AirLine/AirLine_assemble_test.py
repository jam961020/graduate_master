...
# [추가] 저조도 ROI에서 더 강한 CLAHE를 쓸 수 있도록 헬퍼 함수
def apply_clahe_with_params(gray_img: np.ndarray, clip_limit: float, tile_grid: tuple) -> np.ndarray:
...
def _remap01(x: float, a: float, b: float) -> float:
    # a..b -> 0..1
    return _clamp((x - a) / (b - a + 1e-6), 0.0, 1.0)
...
def enhance_color(bgr_image, pre_gray: np.ndarray=None, pre_S: float=None, pre_mean: float=None):
    """
    L*a*b* 색상 공간을 사용하여, 컬러 정보는 유지한 채 밝기 채널만 동적으로 보정합니다.
    아날로그식(연속) 매핑으로 저조도에선 대비를 더, 고조도에선 과대비를 억제합니다.
    """
    # 1. BGR -> L*a*b* 변환 및 채널 분리
    lab_image = cv2.cvtColor(bgr_image, cv2.COLOR_BGR2LAB)
    l_channel, a_channel, b_channel = cv2.split(lab_image)

    # 2. L-Channel(밝기) 전처리 및 지표 계산 + 그레이스케일 기반 지표 산출(의무 적용)
    ...
    if pre_gray is None:
        ...
        gray_blur = pre_gray
    S = float(pre_S) if pre_S is not None else sharp_S(gray_blur)
    mean_l = float(pre_mean) if pre_mean is not None else float(gray_blur.mean())

    # 3. 연속 매핑 파라미터 산출
    # darkness: 0(밝음) ~ 1(어두움)
    darkness = 1.0 - _remap01(mean_l, 70.0, 170.0)
    darkness_s = _smoothstep01(darkness)
    # blur_factor: 0(샤프) ~ 1(블러) — S 작을수록 블러라 가정
    s_norm = _remap01(S, 2e-3, 6e-3)  # 0~1로 정규화(대략)
    blur_factor = 1.0 - _smoothstep01(s_norm)

    # 대비 강도(0~1): 어두움과 블러 정도에 따라 연속 증가
    strength = _clamp(0.15 + 0.75 * darkness_s * (0.5 + 0.5 * blur_factor), 0.0, 1.0)

    # CLAHE clipLimit 연속 결정: 2.0 ~ 12.0
    clip_limit = _lerp(2.0, 12.0, strength)
    # 타일 크기 연속 결정: 8 -> 4 (어두울수록 작게)
    tile_f = _lerp(8.0, 4.0, darkness_s)
    tile_size = int(round(tile_f))
    tile_size = max(4, min(8, tile_size))
    tile_size = tile_size if tile_size % 2 == 0 else tile_size + 1  # 짝수 보정

    # 4. CLAHE 적용 및 블렌딩(원본 L과 가중합으로 과대비 억제)
    clahe_l = apply_clahe_with_params(l_blur, clip_limit=float(clip_limit), tile_grid=(tile_size, tile_size))
    alpha = strength  # 가중치
    enhanced_l = cv2.addWeighted(clahe_l, alpha, l_blur, 1.0 - alpha, 0.0)

    # 5. 보정된 L 채널과 원본 a, b 채널 재결합 -> BGR 반환
...
def find_best_fit_line_ransac(lines_by_algo, roi_w, roi_h, consensus_weight=5):
    ...
    # 2) AirLine 두 프리셋과 기타 알고리즘 픽셀 집합 산출
    ...
    # 3) 카테고리 분류 (요청 가중치 규칙 적용)
    # - 한 에어라인에서만 나옴: weight=3
    # - 두 에어라인에서 동시에 나옴: weight=10 (타 알고리즘과 겹쳐도 동일 가중치 유지)
    # - 한 에어라인 + 다른 선 검출 겹침: weight=5
    # - 다른 선 검출만 나옴: weight=1
    both_air = airline_both  # AirLine_Q ∩ AirLine_QG
    one_air_any = airline_any - airline_both
    one_air_and_other = (airline_any.intersection(other_pixels)) - airline_both
    one_air_only = one_air_any - other_pixels
    other_only = other_pixels - airline_any

    # 4) 픽셀 가중치 풀 조합 (서로 배타 집합으로 구성)
    pixel_pool = []
    pixel_pool.extend(list(other_only) * 1)
    pixel_pool.extend(list(one_air_only) * 3)
    pixel_pool.extend(list(one_air_and_other) * 5)
    pixel_pool.extend(list(both_air) * 10)

    # 디버그 출력
#    print(f"    [DEBUG]   - Pixels: Air_Q={len(air_q)}, Air_QG={len(air_qg)}, Other={len(other_pixels)}")
#    print(f"    [DEBUG]   - Categories: both_air={len(both_air)}, one_air_and_other={len(one_air_and_other)}, one_air_only={len(one_air_only)}, other_only={len(other_only)}")
...
def process(data_dir: Path, out_root: Path, airline_config: dict):
    ...
    for n in list(ALGOS.keys()) + ["AirLine_Q", "AirLine_QG", "RANSAC_combined", "YOLO_only_results", "final_json_results", "Homography_Results", "Pose_Estimation_Vis", "ROI_Crops", "enhance_color"]:
    ...
    for i, p in enumerate(tqdm(imgs, desc="Total Progress")):
        ...
        for cls_h, x1_h, y1_h, x2_h, y2_h in guideline_rois:
            ...
            roi_gray_h = cv2.cvtColor(roi_bgr_h, cv2.COLOR_BGR2GRAY)
            roi_gray_h_blur = cv2.GaussianBlur(roi_gray_h, (3, 3), 0.8)
            S_h = sharp_S(roi_gray_h_blur)
            mean_h = float(roi_gray_h_blur.mean())
            enhanced_h = enhance_color(roi_bgr_h, pre_gray=roi_gray_h_blur, pre_S=S_h, pre_mean=mean_h)
        ...
        for rid, (cls, x1_roi, y1_roi, x2_roi, y2_roi) in enumerate(rois):
            ...
            if cls == 0:
                ...
                cv2.rectangle(ransac_vis, (x1_roi, y1_roi), (x2_roi, y2_roi), (255, 255, 255), 10)
                continue
            ...
            # [신규] ROI 크롭 개별 저장
            roi_crops_dir = out_root / "ROI_Crops"
            roi_name = f"{p.stem}_cls{cls}_roi{rid}_{x1_roi}-{y1_roi}-{x2_roi}-{y2_roi}.png"
            cv2.imwrite(str(roi_crops_dir / roi_name), roi_bgr)
            ...
            roi_bgr_enhanced = enhance_color(roi_bgr, pre_gray=roi_gray_blur, pre_S=S_roi, pre_mean=mean_roi)
            enhanced_image_canvas[y1_roi:y2_roi, x1_roi:x2_roi] = roi_bgr_enhanced
            roi_gray_enhanced = cv2.cvtColor(roi_bgr_enhanced, cv2.COLOR_BGR2GRAY)

            # [임시 저장] ROI 보정본 및 알고리즘별 라인 오버레이 저장
            _roi_dbg_dir = out_root / "ROI_Crops"
            _roi_base = f"{p.stem}_cls{cls}_roi{rid}"
            # 보정본 저장
            cv2.imwrite(str(_roi_dbg_dir / f"{_roi_base}_enh.png"), roi_bgr_enhanced)
            # 알고리즘별 색상 정의 (AirLine 확장 포함)
            _algo_colors = {name: col for name, (_, col) in ALGOS.items()}
            _algo_colors.update({"AirLine_Q": (0, 255, 200), "AirLine_QG": (0, 200, 255)})
            ...
            # AirLine을 두 프리셋으로 동시 실행
            airline_preset_Q = {
                "edgeThresh": -3.0,
                "simThresh": 0.98,
                "pixelNumThresh": airline_config.get("pixelNumThresh", 300),
            }
            airline_preset_QG = {
                "edgeThresh": 1.0,
                "simThresh": 0.75,
            ...
            # [임시 저장] 알고리즘별 ROI 오버레이 이미지 저장
            for _algo_name, _lines in lines_by_algo.items():
                _ov = roi_bgr_enhanced.copy()
                _col = _algo_colors.get(_algo_name, (255, 255, 255))
                if _lines is not None and _lines.size > 0:
                    for _x1, _y1, _x2, _y2 in _lines.astype(int):
                        cv2.line(_ov, (int(_x1), int(_y1)), (int(_x2), int(_y2)), _col, 3)
                cv2.imwrite(str(_roi_dbg_dir / f"{_roi_base}_{_algo_name}.png"), _ov)
            ...
            if cls in [1, 2]:
                ...
                if ransac_line:
                    ...
                    cv2.line(ransac_vis, pt1_global, pt2_global, col, 12)
            ...
            # 이후 클래스 3, 4, 5, 6 처리
                ...
                if len(three_sides) == 3 and longi_to_use:
                    ...
                    if len(final_points) == 4:
                        ...
                        cv2.polylines(ransac_vis, [np.array(final_points, dtype=np.int32)], True, (0,255,0), 12)
                        # RANSAC_combined에도 표시되도록 유지(현재 ransac_vis가 저장됨)
            ...
            if np.all(l_fillet_pt != 0) and np.all(r_fillet_pt != 0):
                ...
                try:
                    ...
                    if H is not None:
                        ...
                        # [추가] 호모그래피 기반으로 Hole 폭 재계산하여 JSON 갱신
                        try:
                            transformed_left_w = None
                            transformed_right_w = None
                            if hole_boxes:
                                for h_box in hole_boxes:
                                    hx1, hy1, hx2, hy2 = h_box
                                    pts = np.array([[[hx1, hy1]], [[hx2, hy1]], [[hx1, hy2]], [[hx2, hy2]]], dtype=np.float32)
                                    tpts = cv2.perspectiveTransform(pts, H).reshape(-1, 2)
                                    top_w = float(np.linalg.norm(tpts[1] - tpts[0]))
                                    bot_w = float(np.linalg.norm(tpts[3] - tpts[2]))
                                    width_warp = (top_w + bot_w) * 0.5
                                    cx = float(np.mean(tpts[:, 0]))
                                    if cx < w / 2:
                                        if (transformed_left_w is None) or (width_warp > transformed_left_w):
                                            transformed_left_w = width_warp
                                    else:
                                        if (transformed_right_w is None) or (width_warp > transformed_right_w):
                                            transformed_right_w = width_warp
                            if transformed_left_w is not None:
                                output_data["is_hole_left"] = 1
                                output_data["hole_left_bounding_width"] = float(transformed_left_w)
                                print(f"  [INFO] Updated left hole width (warped): {transformed_left_w:.2f} px (~{transformed_left_w/20.0:.2f} cm)")
                            if transformed_right_w is not None:
                                output_data["is_hole_right"] = 1
                                output_data["hole_right_bounding_width"] = float(transformed_right_w)
                                print(f"  [INFO] Updated right hole width (warped): {transformed_right_w:.2f} px (~{transformed_right_w/20.0:.2f} cm)")
                        except Exception as e:
                            print(f"  [WARN] Failed to update hole widths via homography: {e}")
        ...
        # cv2.imwrite(str(out_root / "Welding_Filter_Debug" / f"{p.stem}_welding_debug.png"), welding_debug_vis)  # disabled to save space
        ...
        for label, (px, py) in points_to_draw_orig.items():
            if px != 0 and py != 0:
                ...
                cv2.circle(ransac_vis, (px, py), radius=16, color=color, thickness=-1)
                cv2.putText(ransac_vis, label, (px + 22, py + 8), cv2.FONT_HERSHEY_SIMPLEX, 1.6, color, 3)
        
        # [추가] ROI 보정 적용본만 저장 (시연용)
        try:
            cv2.imwrite(str(out_root / "enhance_color" / f"{p.stem}_roi_enh.png"), enhanced_image_canvas)
        except Exception as e:
            print(f"  [WARN] Failed to save ROI enhance_color composite: {e}")
        
        cv2.imwrite(str(out_root / "RANSAC_combined" / f"{p.stem}_ransac.png"), ransac_vis)
        # cv2.imwrite(str(out_root / "enhanced_full_image" / f"{p.stem}_enhanced.jpg"), enhanced_image_canvas)  # disabled to save space
        ...
        # [수정] 알고리즘별 결과 저장 (기존 + AirLine_Q, AirLine_QG 포함)
        save_algo_names = list(ALGOS.keys()) + ["AirLine_Q", "AirLine_QG"]
        base_colors = {name: col for name, (_, col) in ALGOS.items()}
        base_colors.update({
            "AirLine_Q": (0, 255, 200),
            "AirLine_QG": (0, 200, 255),
        })

        for n in save_algo_names:
            vis = bgr.copy()
            all_lines_for_algo = [ln for ln in acc.get(n, []) if ln.size > 0]
            if not all_lines_for_algo:
                cv2.imwrite(str(out_root / n / f"{p.stem}_{n}.png"), vis)
                continue

            for rid_save, lines_save in enumerate(acc.get(n, [])):
                if lines_save.size == 0:
                    continue
                ...
                # AirLine 계열은 클래스 색상 규칙 적용
                if n.startswith("AirLine"):
                    base_col = base_colors.get(n, (0, 255, 255))
                    col = {1: (255, 0, 0), 2: (0, 0, 255)}.get(
                        cls_save, (0, 255, 0) if cls_save in [3, 4, 5, 6] else base_col
                    )
                else:
                    col = base_colors.get(n, (255, 255, 255))
...
def find_3_sides_of_collar(lines_by_algo, roi_w, roi_h, is_left_side):
    # AirLine 두 프리셋 병합 사용, 없으면 기타 알고리즘 폴백 병합
    merged = []
