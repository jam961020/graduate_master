...
def find_3_sides_of_collar(lines_by_algo, roi_w, roi_h, is_left_side):
    ...
    Returns:
        dict with keys: 'top', 'bottom', 'outer_vertical' (각각 [x1,y1,x2,y2])
    """
    # AirLine 두 프리셋 병합
    merged = []
...
def detect_with_full_pipeline(image, params, yolo_detector):
    ...
    # [디버그] ROI 검출 상황 출력
    #print(f"  [PIPELINE] Detected {len(rois)} ROIs: {[cls for cls, *_ in rois]}")
...
def weighted_ransac_line(final_candidates, roi_w, roi_h,
    ...
    - w_length (0~1): weight by line length
    - w_center (0~1): weight by ROI centrality
    - consensus_weight (>=1): boost probability for AirLine lines
    final_candidates: dict[str, list[[x1,y1,x2,y2],...]]
    returns: ((xA,yA),(xB,yB)) or None
    """
    ...
    # Inlier evaluation uses candidate midpoints
    pts = []
    ...
    for _ in range(iters):
        ...
        if inl > best_inl:
            best_inl = inl
            # compute ROI-bounded segment endpoints
            cand = []
            # horizontal borders
            for ty in (0, roi_h):
                if X1[1] != X2[1]:
...
print(f"[AirLine] Q   edge={airline_preset_Q['edgeThresh']:.3f}, sim={airline_preset_Q['simThresh']:.3f}, px={airline_preset_Q['pixelNumThresh']}")
print(f"[AirLine] QG  edge={airline_preset_QG['edgeThresh']:.3f}, sim={airline_preset_QG['simThresh']:.3f}, px={airline_preset_QG['pixelNumThresh']}")
...
def process_guideline_roi(lines_by_algo, roi_bgr, cls, roi_bbox, image_width):
    """
...
w_center = float(params.get('ransac_center_w', 0.5))   # 0~1
w_length = float(params.get('ransac_length_w', 0.5))   # 0~1
w_consensus = int(params.get('ransac_consensus_w', 5)) # >=1
ransac_line = weighted_ransac_line(final_candidates, roi_w, roi_h,
...
def process_collar_roi(lines_by_algo, roi_bgr, cls, roi_bbox, image_width):
    """
    클래스 3~6 (collar) 처리
...
def calculate_collar_points(collar_sides, left_longi_res, right_longi_res, image_width):
    ...
    if collar_center_x < image_width / 2:
        target_points = sorted_by_x[2:]  # 오른쪽 두 점
    else:
        target_points = sorted_by_x[:2]  # 왼쪽 두 점
